...
- **Color:** dos escalones de papel cálido y solo dos —la hoja es el claro (`--paper: #fbf9f4`) y **la placa un escalón más profundo** (`--surface: #f5f1e8`), así una placa se asienta en la página en vez de levantarse—, tinta casi negra (`#1c1a17`) y un solo acento, óxido (`#b3401e`), que marca lo que se puede seguir (subrayados, cuentas) y lo que está vivo. Sin sombras en ningún sitio. Todo en `:root` al principio de `src/styles/letter.css`; ahí queda anotado por qué la inversión (hoja profunda, placa clara) se probó y se descartó.
...
**La estructura de estas páginas es un solo rectángulo repetido, la placa** (borde de un píxel, esquina de 2 px, papel un escalón más profundo, un único padding), a la manera de [gulipad.com](https://www.gulipad.com/): la nota bajo el nombre de una sección, cada tarjeta, la estantería, cada lista archivada, la hoja de datos y cada principio se dibujan igual. La lista de selectores está en un solo sitio, `src/styles/letter.css` → "The plate"; **un bloque nuevo se añade a esa lista, no se escribe una segunda caja.** Las secciones no se separan con una caja más pesada sino con un filete que va del nombre de la sección al margen derecho (`.part__head`); un nivel por debajo —una temática de `/interests`— la etiqueta va sola. El filete bajo el masthead y el de encima del pie son el marco de la página. La carta de la portada no lleva placas: es prosa desnuda. Ver [DESIGN.md](DESIGN.md) → "Superficies — la placa".

#### Proyectos (`/projects`)

Una hoja ancha (`<Topic wide>`, medida de 1080 px) con dos cuadrículas, a la manera de [gulipad.com](https://www.gulipad.com/): **proyectos activos** (`active: true`), dos por fila, cada uno con su imagen 16:9 (`image`), título, etiquetas en chip, un resumen y las viñetas con lo concreto (`points`); y **otros proyectos**, tres por fila, con título, etiquetas y una línea. Las imágenes viven en `public/projects/`. En pantallas estrechas las cuadrículas bajan a dos y a una columna.

Cada tarjeta es una placa y **llena su fila** (`height: 100%`), así dos tarjetas con distinta cantidad de texto se cuadran entre sí. La imagen llega a sangre hasta el borde de la placa (márgenes negativos de `--pad`), que le hace de marco; el crédito opcional (`image.credit`) baja al pie de la tarjeta con su propio filete, en vez de ir bajo la imagen, para que los títulos de todas las tarjetas empiecen en la misma línea.
